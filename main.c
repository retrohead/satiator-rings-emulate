...
void launch_game(const char *filename) {
    ...
    int ret;
    int isemu = 0;
    if (strcasecmp(&filename[len-5], ".desc")) {
        ret = image2desc(filename, "out.desc");
        if (ret) {
...
char pathbuf[512];
void move_to_autoboot_directory(void)
{
    char * pathfile = "autobootdesc.txt";
    s_chdir("/");
    // open options ini for reading
    char statbuf[280];
    s_stat_t *st = (s_stat_t*)statbuf;
    int fr = s_stat(pathfile, st, sizeof(statbuf)-1);
    fr = s_open(pathfile, FA_READ | FA_OPEN_EXISTING);
    strcpy(pathbuf, "");
    char c[2];
    int bytesRead = 0;
    ...
    if(strlen(pathbuf) < 512)
        pathbuf[strlen(pathbuf)] = '\0';
    s_close(fr);
    // remove the autoboot file
    s_unlink("autobootdesc.txt");
...
void try_autoboot(void) {
    ...
    if (file_exists("autoboot.iso")) {
        // autoboot text file for rings menu
        if(file_exists("autobootdesc.txt"))
        {
            move_to_autoboot_directory();
            launch_game("emu.desc");
        } else
        {
