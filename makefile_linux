...
default: $(NEWLIB_LIBDIR)/libc-nosys.a $(IAPETUS_LIBDIR)/libiapetus.a out/emulate.bin

CC_DEPS = $(NEWLIB_LIBDIR)/libc-nosys.a

out/emulate.bin: ip.bin out/emulate_code.bin
	cat $^ > $@

out/%_code.bin: out/%.elf
	$(OBJCOPY) -O binary -R .noload $< $@

out/emulate.elf: menu.ld $(OBJS) $(IAPETUS_LIBDIR)/libiapetus.a $(NEWLIB_LIBDIR)/libc-nosys.a
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ -T menu.ld -Wl,-Map=out/emulate.map $(OBJS) -liapetus -lc-nosys -lgcc
